    ...
    mSelectionModel{nullptr}
{
    ...
    connect(ui->previousButton, &QPushButton::clicked, [this](){
        auto currentModelIndex = this->mSelectionModel->currentIndex();
        auto previousModelIndex = this->mSelectionModel->model()
    ...
    connect(ui->nextButton, &QPushButton::clicked, [this]() {
        auto currentModelIndex = mSelectionModel->currentIndex();
        auto nextModelIndex = mSelectionModel->model()
                ->index(currentModelIndex.row() + 1, 0);
...
void PictureWidget::deletePicture()
{
    ...
    // Try to select previous picture
...
void PictureWidget::loadPicture(const QItemSelection& selected)
{
    ...
    mPixmap = QPixmap(mModel->data(current,
